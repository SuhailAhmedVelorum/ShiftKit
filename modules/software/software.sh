...
software_install_ghostty() {
    ...
    local repo=$(python3 -c "import yaml; print(yaml.safe_load(open('$CONFIG_FILE'))['software']['ghostty']['repo'])")
    
    # Create a temporary directory for the build
    local temp_dir=$(mktemp -d)
    
    # Clone the repository
    git clone "$repo" "$temp_dir/ghostty"
    
    # Build and install
    cd "$temp_dir/ghostty"
    
    # Install dependencies
    sudo apt-get install -y meson libgtk-3-dev libpango1.0-dev libglib2.0-dev libfontconfig-dev libfreetype-dev
