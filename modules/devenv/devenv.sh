...
devenv_install_tools() {
    ...
    local dev_tools="build-essential git curl wget vim neovim tmux zsh python3-pip python3-venv"
...
# Function to set up Node.js development environment
devenv_setup_node() {
    if ! python3 -c "import yaml; print(yaml.safe_load(open('$CONFIG_FILE'))['devenv']['node']['enabled'])" | grep -q "True"; then
        log "INFO" "Skipping Node.js development environment setup (disabled in config)"
        return 0
    fi
    
    log "INFO" "Setting up Node.js development environment..."
...
# Function to set up Go development environment
devenv_setup_go() {
    if ! python3 -c "import yaml; print(yaml.safe_load(open('$CONFIG_FILE'))['devenv']['go']['enabled'])" | grep -q "True"; then
        log "INFO" "Skipping Go development environment setup (disabled in config)"
        return 0
    fi
    ...
    local go_version=$(python3 -c "import yaml; print(yaml.safe_load(open('$CONFIG_FILE'))['devenv']['go']['version'])")
    
    # Download and install Go
    local temp_dir=$(mktemp -d)
    wget "https://golang.org/dl/go${go_version}.linux-amd64.tar.gz" -O "${temp_dir}/go.tar.gz"
...
# Function to set up Rust development environment
devenv_setup_rust() {
    if ! python3 -c "import yaml; print(yaml.safe_load(open('$CONFIG_FILE'))['devenv']['rust']['enabled'])" | grep -q "True"; then
        log "INFO" "Skipping Rust development environment setup (disabled in config)"
        return 0
    fi
